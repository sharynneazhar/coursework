...
int priqueue_offer(priqueue_t *q, void *ptr)
{
				...
				// Begin traversal to find the right index to insert the
				// new element into (prioritize)
				int indexToReturn = 0;
				...
				while (currNode != NULL)
				{
								// if the new element has greater priority
								if (q->m_comparer(newNode->m_ptr, currNode->m_ptr) < 0)
								{
												// shift all elements down by swapping
												while (currNode != NULL)
												{
																tempNode = currNode->m_ptr;
